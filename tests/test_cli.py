...
def test_cli_dedupe(capsys):
    ...
    with patch.object(sys, 'argv', ["smart-organizer", "dedupe", "--root", "."]):
        with patch("smart_file_organizer.cli.main.DirectoryScanner") as MockScanner, \
             patch("smart_file_organizer.cli.main.DuplicateFinder") as MockDedupe:
...
def test_cli_organize_dry_run(capsys):
    ...
    with patch.object(sys, 'argv', ["smart-organizer", "organize", "--root", ".", "--by-ext"]):
        with patch("smart_file_organizer.cli.main.DirectoryScanner") as MockScanner, \
             patch("smart_file_organizer.cli.main.Organizer") as MockOrg:
...
def test_cli_verbose_and_errors(capsys):
    ...
    """
    with patch.object(sys, 'argv', ["smart-organizer", "--verbose", "scan", "--root", "."]):
        with patch("smart_file_organizer.cli.main.DirectoryScanner") as MockScanner:
