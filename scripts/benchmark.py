...
def run_dedupe(parallel: bool) -> float:
    ...
    if parallel:
        ...
        with patch("smart_file_organizer.use_cases.dedupe.ProcessPoolExecutor", return_value=SerialExecutor()):
            finder = DuplicateFinder(container.hasher)
            finder.find_duplicates(files)
...
def plot_results(serial_time, parallel_time):
    ...
    plt.title(f'Deduplication Performance ({FILE_COUNT} files @ {FILE_SIZE_KB}KB)', fontsize=14)
    ...
    # Add system info box
    plt.figtext(0.15, 0.02, sys_info, fontsize=9, 
